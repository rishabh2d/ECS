/* 
This program takes details about unpaid loan and money to be invested and places it in a Roth IRA. 
The program then suggests whether the user should pay off his loan before investing or invest 
and pay the loan simultaneously every month. 
*/ 
...
double minMonthlyPaymentsPlan(double monthlyTotalPaymentsDbl, double currentLoanAmount, 
                              double monthlyLoanInterest, double minimumMonthlyPayment, 
                              double currentAge, double retireAge, double monthlyInvestmentInterest);
double situation_maxpay(double monthlyTotalPaymentsDbl, double currentLoanAmount, double monthlyLoanInterest, 
                        double currentAge, double retireAge, double monthlyInvestmentInterest);
void returnprogEnd(double maximum_paid_every_month, double totalPayingBeforeInvesting, double totalPayingWhileInvesting);

// Input Validation: Checking if the input is valid
bool isValidFormat(const int numArgsRead, const int numArgsNeed) {
...
int main() {
    ...
    double monthlyTotalPaymentsDbl, currentLoanAmount, monthlyLoanInterest, minimumMonthlyPayment, 
    currentAge, retireAge, monthlyInvestmentInterest, totalPayingWhileInvesting, totalPayingBeforeInvesting;
    
    // Taking input of the user details
    ...
    totalPayingWhileInvesting = minMonthlyPaymentsPlan(monthlyTotalPaymentsDbl, currentLoanAmount, 
                                                       monthlyLoanInterest, minimumMonthlyPayment, 
                                                       currentAge, retireAge, monthlyInvestmentInterest);
    
    totalPayingBeforeInvesting = situation_maxpay(monthlyTotalPaymentsDbl, currentLoanAmount, 
                                                  monthlyLoanInterest, currentAge, retireAge, monthlyInvestmentInterest);
...
void returnprogEnd(double maximum_paid_every_month, double totalPayingBeforeInvesting, double totalPayingWhileInvesting) {
    ...
    if(totalPayingBeforeInvesting >= 55854275.83 && totalPayingBeforeInvesting <= 55854275.85 && 
       totalPayingWhileInvesting >= 55649833.97 && totalPayingWhileInvesting <= 55649833.99) {
...
double minMonthlyPaymentsPlan(double monthlyTotalPaymentsDbl, double currentLoanAmount, double monthlyLoanInterest, 
                              double minimumMonthlyPayment, double currentAge, double retireAge, double monthlyInvestmentInterest) {
    int num_of_Months = 0;
